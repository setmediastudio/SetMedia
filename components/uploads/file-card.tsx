  ...
  return (
    <Card className="group overflow-hidden hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
      <div className="relative aspect-square bg-muted overflow-hidden cursor-pointer" onClick={() => onView?.(upload)}>
        {isImage && !imageError ? (
          <img
            src={upload.sdPublicUrl || upload.publicUrl || "/placeholder.svg"}
        ...
        <div className="flex items-start justify-between mb-2">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {formatFileSize(upload.sdFileSize || upload.fileSize)}
              {upload.sdPublicUrl && <span className="text-xs text-primary/70 ml-1">(SD)</span>}
            </p>
          </div>
          <DropdownMenu>
